...
func WalkStatHandler(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
	...
	if err := c.WithRenameRLock(func() error {
		...
		for _, name := range req.Path {
			...
			// TODO(ayushranjan): A heap allocated defer? Really?
...
func RenameAtHandler(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
	...
	err = c.WithRenameLock(func() error {
		...
		oldParent.upgradeMu.RLock()
		defer oldParent.upgradeMu.RUnlock()

		newDir.upgradeMu.RLock()
		defer newDir.upgradeMu.RUnlock()
...
func GetXattrHandler(c *lisafs.Connection, comm lisafs.Communicator, payloadLen uint32) (uint32, []int, error) {
	...
	if fd.isControlFD {
		...
		shouldReadLock, err := fd.ensureReadableOrWritable()
		if err != nil {
			return 0, nil, err
		}
		if shouldReadLock {
...
//
// FDs opened via the Open RPCs are not control FDs and can not be upgraded.
// Their open mode is immutable throughout thier lifetime.
//
...
func (fd *FD) upgrade() error {
	...
	upgradedFD, err := unix.Openat(int(procSelfFD.FD()), strconv.Itoa(fd.no), flags&^unix.O_NOFOLLOW, 0)
	if err != nil {
		return err
	}
