...
func MqOpen(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	queue, err := r.FindOrCreate(t, name, rOnly, wOnly, readWrite, create, exclusive, block, statusFlags(flag, rOnly, wOnly, readWrite), linux.FileMode(mode), attrPtr)
	if err != nil {
		return 0, nil, err
	}
...
// statusFlags constructs status flags for a message queue file description.
func statusFlags(flags int32, rOnly, wOnly, readWrite bool) uint32 {
	status := uint32(flags & linux.O_NONBLOCK)
	switch {
	case rOnly:
		return status | linux.O_RDONLY
	case rOnly:
		return status | linux.O_WRONLY
	case rOnly:
		return status | linux.O_RDWR
	default:
		return status
	}
}
