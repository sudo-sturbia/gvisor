...
type Stats struct {
	// DroppedPackets is the number of packets dropped by transport endpoints for
	// various reasons.
	// TODO(https://gvisor.dev/issues/5986): Make this stat less ambiguous.
	DroppedPackets *StatCounter

	// NIC breaks out NIC-specific stats. This is an aggregation of every NIC's
	// NICStats. These stats should typically not be incremented from here, but
	// from the relevant NIC.
	NIC NICStats

	// ICMP breaks out ICMP-specific stats (both v4 and v6). This is an
	// aggregation of every NetworkEndpoint's ICMPStats. These stats should not be
	// incremented using this field, but using the relevant NetworkEndpoint
	// multicounters.
	ICMP ICMPStats

	// IGMP breaks out IGMP-specific stats. This is an aggregation of every
	// NetworkEndpoint's IGMPStats. These stats should not be incremented using
	// this field, but using the relevant NetworkEndpoint multicounters.
	IGMP IGMPStats

	// IP breaks out IP-specific stats (both v4 and v6). This is an aggregation of
	// every NetworkEndpoint's IPStats. These stats should not be incremented
	// using this field, but using the relevant NetworkEndpoint multicounters.
	IP IPStats

	// ARP breaks out ARP-specific stats. This is an aggregation of every
	// NetworkEndpoint's ARPStats. These stats should not be incremented using
	// this field, but using the relevant NetworkEndpoint multicounters.
	ARP ARPStats
