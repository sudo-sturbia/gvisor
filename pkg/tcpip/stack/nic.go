...
//
// Returns true if the packet needs may be further processed; false if the
...
func (n *nic) WritePacket(r *Route, protocol tcpip.NetworkProtocolNumber, pkt *PacketBuffer) (int, tcpip.Error) {
	if needsWriting, packetsWritten, err := n.maybePerformOffloadableTasks(r, protocol, pkt); !needsWriting {
